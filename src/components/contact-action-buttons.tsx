...
function MessagePickerPopover({ onClose }: { onClose: () => void }) {
  return (
    <>
      ...
      <div className="absolute right-0 top-full z-50 mt-1 w-40 rounded-2xl border border-border/50 glass p-1 shadow-lg">
        <PickerItem
          label="SMS"
          onClick={() => {
            openSms();
...
function ManualPickerPopover({
  ...
  return (
    <>
      <div className="fixed inset-0 z-50" onClick={onClose} />
      <div className="absolute right-0 top-full z-50 mt-1 w-36 rounded-2xl border border-border/50 glass p-1 shadow-lg">
        {options.map((opt) => (
          <button
