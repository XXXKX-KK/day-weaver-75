...
// ─── Gauge SVG ───────────────────────────────────────────────────
const Gauge: React.FC<{ pct: number; arcColor: string; arcColorLight: string }> = ({
  pct,
  arcColor,
  arcColorLight,
}) => {
...
/** Formats the day being summarised — not "now", which would mislabel every
 *  past day opened from the stats grid. */
const dateLabel = (iso?: string) =>
  new Intl.DateTimeFormat("pl-PL", { weekday: "long", day: "numeric", month: "long" }).format(
    iso ? new Date(`${iso}T12:00:00`) : new Date(),
  );
...
interface Props {
  tasks: Task[];
  onClose: () => void;
  /** ISO date of the day being shown. Defaults to today. */
  date?: string;
  /** That date has no row in `days` at all. */
  noPlan?: boolean;
}

export const PodsumowanieScreen: React.FC<Props> = ({ tasks, onClose, date, noPlan }) => {
    ...
    >
      ...
      >
        ...
        {noPlan ? (
          ...
          >
            <div
              style={{ fontSize: 16, fontWeight: 700, color: "var(--foreground)", marginBottom: 6 }}
            >
              Tego dnia nie było planu
            </div>
            <div style={{ fontSize: 14, color: "var(--muted-foreground)", lineHeight: 1.5 }}>
              Dzień nie został rozpoczęty, więc nie ma czego podsumować.
            </div>
          </div>
        ) : (
          <>
                  ...
                  >
                    ...
                    >
                      <svg
                        ...
                        style={{
                          strokeDasharray: 20,
                          animation: `checkDraw 0.4s ease-out ${0.6 + i * 0.08}s both`,
                        }}
                      >
                    ...
                    <div style={{ position: "relative", flex: 1, minWidth: 0, overflow: "hidden" }}>
                      ...
                      <div
                        style={{
                          ...
                          borderRadius: 1,
                          animation: `strikeIn 0.35s ease-out ${0.7 + i * 0.08}s both`,
                          width: "100%",
                        }}
                      />
                    </div>
