...
begin
  ...
  -- 2) kandydaci: aktywne rutyny pasujace do dnia tyg. + zadania open (scheduled<=dzis).
  --    kolejnosc = JEDNA wspolna sekwencja wg recznego position (rutyny, potem zadania),
  --    z globalnym offsetem po istniejacych pozycjach (idempotentnie, bez per-block).
  with candidates as (
  ...
  ranked as (
    select c.*,
      ...
      + row_number() over (
          order by c.source_rank, c.src_pos asc nulls last, c.src_created asc
        ) as position
    from candidates c
  ),
  inserted as (
